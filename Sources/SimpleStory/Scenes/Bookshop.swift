...
enum Bookshop {
  static let scenes: [RawScene<SimpleStory>] = [
    ...
    Bookshop_ShowPhotoAgain.raw,
    Bookshop_AboutTheShop.raw,
    Bookshop_TheFeelingShop.raw,
  ]
}
        ...
        if $0.script.didNarrate(.didShowPhotoOnce) {
          "Show photo again".onSelect {
            .tell {
              ...
              .runThrough(Bookshop_ShowPhotoAgain())
            }
          }
        } else {
          "Show photo".onSelect {
        ...
        if !$0.script.didNarrate(.didWitnessALargerWorld) {
          "Ask about the shop".onSelect {
            .tell {
              ...
              .runThrough(Bookshop_AboutTheShop())
            }
          }
        }
...
struct Bookshop_ShowPhotoAgain: SceneType {
  var steps: Steps {
    "'Have you seen this person?'"
...
struct Bookshop_AboutTheShop: SceneType {
  var steps: Steps {
    "'What do you mean?'"
    ...
    DO.check {
      if $0.script.narrated[.didThinkAboutTheBookshopFeelings, default: 0] >= 3 {
        .tell {
          ...
          .replaceWith(Bookshop_TheFeelingShop())
        }
      } else {
        .tell {
...
struct Bookshop_TheFeelingShop: SceneType {
  var steps: Steps {

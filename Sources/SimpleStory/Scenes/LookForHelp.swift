...
enum LookForHelp {
  static let scenes: [RawScene<SimpleStory>] = [
    LookForHelp_Main.raw,
    LookForHelp_Conversation.raw,
  ]
}

struct LookForHelp_Main: SceneType {
  var steps: Steps {
    ...
    DO.then { .replaceWith(LookForHelp_Conversation()) }
  }
}

struct LookForHelp_Conversation: SceneType {
  ...
  var steps: Steps {
    ...
    DO.then { .transitionTo(Apartment7_TheApartment()) }
  }
}
